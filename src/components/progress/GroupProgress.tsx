...
export function GroupProgress({ progressList }: GroupProgressProps) {
  if (!progressList.length) {
    return (
      <p className="text-sm text-stone-400 italic">Noch keine Fortschritte eingetragen.</p>
    )
  }
    ...
    <div className="flex flex-col gap-3">
      ...
      <div>
        <div className="flex justify-between mb-1.5">
          <span className="text-xs text-stone-500 font-medium">Gruppenfortschritt</span>
          <span className="text-xs font-semibold text-brand-600">{Math.round(avg)}%</span>
        </div>
